 ...
 */
func ReConstructBinaryTree( pre []int ,  vin []int ) *common.TreeNode {
	// write code here
	...
	if len(pre) == 1 {
		var root = common.TreeNode{pre[0], nil,nil}
		return &root
	}
	rootVal := pre[0]
	temp := 0
...
//通过代码
func BuildTree2(preorder []int, inorder []int) *common.TreeNode {
	if len(preorder) == 0 {
		return nil
	}
